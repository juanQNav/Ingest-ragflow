...
def download_file(session, file_url, output_path, file_name, total_size_in_bytes, position=0):
    ...
    if file_response.status_code == 200:
        ...
        tqdm.write(f"Error {file_response.status_code} while downloading the file.")
...
def retrieve_item_file(session, base_url, base_url_rest, item_id, folder_path, position):
    item_url = f"{base_url_rest}/items/{item_id}?expand=bitstreams"
    response = session.get(item_url)
        ...
        if bitstreams:
            ...
            if file_url:
                ...
                if not os.path.exists(file_path):
                    total_size_in_bytes = bitstreams[0].get('sizeBytes', 0)
                    download_file(session, f'{base_url}{file_url}', folder_path, file_name, total_size_in_bytes, position)
                else:
                    tqdm.write(f"[INFO] File {file_name} already exists, skipping download...")
                return file_path
    return None
